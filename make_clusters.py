...
from GAN_train import UNIT_Train_cl
from GAN_test import UNIT_Test_cl
...
if __name__ == '__main__':
    # 1. load csv data
    datas = []
    building_names = ['CP4', 'DEH', 'OIE']
    for i in range(len(building_names)):
        datas.append(pd.read_csv('./raw_data/{}.csv'.format(building_names[i])))
    ...
    * task_dict and metadata_task_list length are same, of course.
    ...
    # 4.1 save clustering data result
    ...
    # 4.2 train model for all clustering (attention: not for metadata, it's cluster)
    ...
    for cluster_id, metadata_id_list in metadata_clustering_dict.items():
        ...
        save_dict = read_data(data=df_this_cluster, cluster=cluster_id, context_a=context_a, context_b=context_b)
        gen_a, gen_b = UNIT_Train_cl(cluster=cluster_id, context_a=context_a, context_b=context_b, save_dict=save_dict)
    ...
    cluster = metadata_clustering_models_dict[check_clustering_index]
