...
def UNIT_Train_cl(cluster, context_a, context_b, save_dict):
    ...
    for epoch in range(num_epochs):
        ...
        min_mae = 10000
        if (epoch + 1) % 10 == 0:
            this_mae = check_training_result(gen_a, gen_b, cluster, context_a, context_b, save_dict)
            if this_mae < min_mae:
                best_gen_a = gen_a
                best_gen_b = gen_b
                print('Models saved at ./models')

    return best_gen_a, best_gen_b
